...
import hr.gregl.dal.Repository0708;

public class SqlRepository0708 implements Repository0708 {
    ...
    @Override
    public int createArticle(Article0708 article) throws Exception {
        ...
        try (Connection con = dataSource.getConnection(); CallableStatement stmt = con.prepareCall(CREATE_ARTICLE)) {
            ...
            stmt.setString(PUBLISHED_DATE, article.getPublishedDate().format(Article0708.DATE_FORMATTER));
    ...
    @Override
    public void createArticles(List<Article0708> articles) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection(); CallableStatement stmt = con.prepareCall(CREATE_ARTICLE)) {

            for (Article0708 article : articles) {
                stmt.setString(TITLE, article.getTitle());
                stmt.setString(LINK, article.getLink());
                stmt.setString(DESCRIPTION, article.getDescription());
                stmt.setString(PICTURE_PATH, article.getPicturePath());
    ...
    @Override
    public void updateArticle(int id, Article0708 data) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection(); CallableStatement stmt = con.prepareCall(UPDATE_ARTICLE)) {
            ...
            stmt.setString(PUBLISHED_DATE, data.getPublishedDate().format(Article0708.DATE_FORMATTER));
    ...
    @Override
    public Optional<Article0708> selectArticle(int id) throws Exception {
        DataSource dataSource = DataSourceSingleton.getInstance();
        try (Connection con = dataSource.getConnection(); CallableStatement stmt = con.prepareCall(SELECT_ARTICLE)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                if (rs.next()) {
                    return Optional.of(new Article0708(
                            rs.getInt(ID_ARTICLE),
                            rs.getString(TITLE),
                            rs.getString(LINK),
                            rs.getString(DESCRIPTION),
                            rs.getString(PICTURE_PATH),
                            LocalDateTime.parse(rs.getString(PUBLISHED_DATE), Article0708.DATE_FORMATTER)));
                }
            }
        }
    ...
    @Override
    public List<Article0708> selectArticles() throws Exception {
        List<Article0708> articles = new ArrayList<>();
        DataSource dataSource = DataSourceSingleton.getInstance();
